...
def _stealth_headers() -> Dict[str, str]:
    ...
    title = os.getenv("OPENROUTER_TITLE", "AegisGateway")
    return {
        "HTTP-Referer": ref,
        "X-Title": title,
    }
...
async def chat_completion(payload: Dict[str, Any]) -> Union[Dict[str, Any], AsyncGenerator[str, None]]:
    ...
    # Заменяем устаревшую модель на актуальную
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        async def _gen():
            # Обрабатываем поток событий от OpenRouter
            try:
                async for line in resp.aiter_lines():
                    if line.strip():
                        # Убираем "OPENROUTER PROCESSING" из начала строки, если есть
